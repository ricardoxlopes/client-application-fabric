...
var MarkdownRenderer = (function () {
    ...
    MarkdownRenderer.prototype.renderRelease = function (release) {
        ...
        if (categoriesWithCommits.length === 0)
            return "";
        var releaseTitle = release.name === UNRELEASED_TAG ? "Unreleased" : release.name;
    ...
    MarkdownRenderer.prototype.renderContributionsByPackage = function (commits) {
        ...
        return packageNames.map(function (packageName) {
            var commits = commitsByPackage[packageName];
            return "* " + packageName + "\n" + _this.renderContributionList(commits, "  ");
        }).join("\n");
    };
    MarkdownRenderer.prototype.renderPackageNames = function (packageNames) {
        return packageNames.length > 0
            ? packageNames.map(function (pkg) { return "`" + pkg + "`"; }).join(", ")
            : "Other";
    };
    ...
    MarkdownRenderer.prototype.groupByCategory = function (allCommits) {
        return this.options.categories.map(function (name) {
            var commits = allCommits
                .filter(function (commit) { return commit.categories && commit.categories.indexOf(name) !== -1; });
